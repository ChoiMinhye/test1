...
### 1번
 초기에는 재귀함수로 Facotorial 함수를 작성하기 위하여 1~n-1까지를 곱한 값에 n을 곱하는 방식으로 코드를 작성하였었다.  
 이때 종료 조건을 n==1 or 0일 경우 1을 반환하고 종료하도록 하였는데,  
 코드리뷰를 통하여 종료조건이 n==0인 경우에만 설정해도 무방하므로  
 프로그램 속도를 빠르게 하기 위해서는 수정하는 것이 좋겠다는 의견이 있어 이를 수정하였다.  
